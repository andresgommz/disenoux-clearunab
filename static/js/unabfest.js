...
(function(){
  // account button: si hay usuario -> userpage.html, si no -> login.html
  const account = document.querySelector('.account');
  if(account){
    account.addEventListener('click', (e)=>{
      e.preventDefault();
      let user = null;
      try{ user = JSON.parse(localStorage.getItem('user.name')); }catch{}
      location.href = user ? 'userpage.html' : 'login.html';
    });
  }
  ...
  const userEvKey = 'user.events';
  const calKey = 'cal.items';

  // Recorrer cards y atar botón de registro
  document.querySelectorAll('.card').forEach(card=>{
    const btn = card.querySelector('.btn-outline');
    if(!btn) return;

    function readEvent(){
      const title = (card.querySelector('h3')?.textContent||'Evento').trim();
      const metaText = (card.querySelector('.meta')?.textContent||'').trim();
      // extraer fecha formato dd/mm/yyyy si existe
      let fecha = '';
      const m = metaText.match(/(\d{2}\/\d{2}\/\d{4})/);
      if(m){ const [d,mm,y] = m[1].split('/'); fecha = `${y}-${mm}-${d}`; }
      else {
        // intentar año-mes-dia
        const m2 = metaText.match(/(\d{4}-\d{2}-\d{2})/);
        if(m2) fecha = m2[1];
      }
      const horaM = metaText.match(/(\d{1,2}:\d{2})/);
      const hora = horaM ? horaM[1] : '08:00';
      const desc = metaText;
      return { titulo: title, fecha: fecha || new Date().toISOString().slice(0,10), hora, descripcion: desc, img: '' };
    }

    function isRegistered(ev){
      const arr = storageGet(userEvKey);
      return arr.some(x => x.titulo === ev.titulo && x.fecha === ev.fecha);
    }

    // marcar estado inicial
    const ev = readEvent();
    if(isRegistered(ev)){ btn.textContent = 'Registrado'; btn.disabled = true; }
    ...
    btn.addEventListener('click', (e)=>{
      ...
      const ev2 = readEvent();
      ev2.id = crypto.randomUUID ? crypto.randomUUID() : ('ev_'+Date.now());

      const arr = storageGet(userEvKey);
      if(!arr.some(x => x.titulo === ev2.titulo && x.fecha === ev2.fecha)){
        arr.push(ev2);
        storageSet(userEvKey, arr);
      }

      // añadir también a cal.items si no existe
      const cal = storageGet(calKey);
      if(!cal.some(c => c.titulo===ev2.titulo && c.fecha===ev2.fecha && c.hora===ev2.hora)){
        cal.push({ id: ev2.id, titulo: ev2.titulo, fecha: ev2.fecha, hora: ev2.hora||'08:00', nota: ev2.descripcion||'' });
        storageSet(calKey, cal);
      }

      btn.textContent = 'Registrado';
      btn.disabled = true;
      alert('Evento registrado en tu panel.');
    });
  });
})();
